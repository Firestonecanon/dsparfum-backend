...
export default function ContactSection() {
  ...
  // Fonction pour charger les données de commande ou panier
  const loadOrderData = () => {
    ...
    // 2. Vérifier localStorage (panier universel)
    const cartRaw = localStorage.getItem('dsparfum_cart') || localStorage.getItem('cart');
    if (cartRaw) {
      try {
        const cart = JSON.parse(cartRaw);
        if (Array.isArray(cart) && cart.length > 0) {
          // Formatage du panier pour injection
          const cartMessage = cart.map(item => `- ${item.name || item.nom || 'Produit'} (${item.quantity || item.qte || 1}x) : ${(item.price || item.prix || 0)} €`).join('\n');
          const total = cart.reduce((sum, item) => sum + ((item.price || item.prix || 0) * (item.quantity || item.qte || 1)), 0);
          const message = `PANIER CLIENT:\n${cartMessage}\nTotal: ${total.toFixed(2)} €`;
          setFormData(prev => {
            const alreadyInjected = prev.message && prev.message.includes(message);
            return {
              ...prev,
              subject: prev.subject || 'Commande D&S Parfum',
              message: alreadyInjected
                ? prev.message
    ...
    console.log('❌ Aucune commande ou panier trouvé');
    return false;
  };
    ...
    emailjs.send(serviceID, templateID, templateParams, userID)
      .then(async () => {
        ...
        try {
          ...
          console.log('✅ Contact enregistré dans l\'admin');
        } catch (error) {
